...
const copy = {
  zh: {
    ...
    title: "多語智慧<br><span>回收挑戰</span>", lead: "先用凍好查尋找答案，再完成5道生活回收任務！",
    ...
    correctionEyebrow: "再查一次就完成", correctionTitle: "還差一點點，完成訂正後即可參加抽獎！", firstScore: "首次作答",
    correctionCopy: "請再次使用「竹縣回收凍好查」查詢下列品項，只需重新挑戰答錯的題目。", startCorrection: "開始訂正答錯題目", retryQuestion: "再查一次",
    progress: "第 {current} 題／共 {total} 題", score: "{score} 分", correct: "答對了！", wrong: "再認識一下",
    completeEyebrow: "挑戰完成", completeTitle: "你完成了多語智慧回收挑戰！",
    completeCopy: "點擊下方按鈕返回LINE，即可取得抽獎資格並參加線上抽獎。",
...
const questions = [
  {
    icon: "🍾",
    zh: { term: "玻璃瓶", q: "根據凍好查的回覆，廢玻璃瓶應如何分色回收？", a: ["依透明、褐色、綠色分開回收", "只分深色與淺色", "不必分色，全部混在一起"], correct: 0, note: "玻璃瓶請先沖洗乾淨，再依透明、褐色及綠色分開回收。" },
  ...
  {
    icon: "🥡",
    zh: { term: "紙餐盒", q: "根據凍好查的回覆，吃完便當後，紙餐盒應該怎麼處理？", a: ["清除剩食、簡單清潔後，以紙容器類回收", "連同剩食直接回收", "一定只能丟一般垃圾"], correct: 0, note: "先清除剩食並簡單清潔，再以紙容器類交付回收；剩食則依規定分類。" },
  ...
  {
    icon: "🧪",
    zh: { term: "農藥瓶", q: "根據凍好查的回覆，廢農藥容器回收前應如何處理？", a: ["三沖三洗，沖洗液倒回施藥桶使用", "以清潔劑清洗後倒入水溝", "不需清洗，直接混入一般回收物"], correct: 0, note: "廢農藥容器應完成三沖三洗，並將沖洗液倒回施藥桶使用，再交付回收。" },
  ...
  {
    icon: "🧯",
    zh: { term: "高壓容器", q: "根據凍好查的回覆，高壓容器安全回收的正確步驟是什麼？", a: ["確認用盡、獨立打包、加註警語、交付資收", "自行打洞、壓扁後丟一般垃圾", "保留內容物並與其他回收物混裝"], correct: 0, note: "請記住「完、包、警、收」：確認用盡、獨立打包、加註警語，再交付資源回收；不要自行打洞。" },
